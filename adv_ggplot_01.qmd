...
plot1 <- ggplot(weather_w_months) +
    ...
    geom_point(data = record_highs, aes(x = dateInYear, y = High+0.8), shape = 25, fill = "black")+
    geom_text(data = record_highs, aes(x = dateInYear, y = High+5.5, label = RecordText), size = 2)+
    geom_text(data = special_record_high, aes(x = dateInYear, y = High+11, label = RecordText), size = 2)+
...
plot2 <- weather %>% 
  ...
  geom_text(data = month_ends_feb, aes(x = dateInYear-5, y = CulmPrec+0.5, label = CulmPrec), size = 3)+
  geom_point(data = record_precips, aes(x = dateInYear, y = CulmPrec+0.2), shape = 25, fill = "black")+
  geom_vline(data = month_starts, aes(xintercept = dateInYear), color = "#b2b2b2", linewidth = 0.3)+
  theme_classic()+
  theme(axis.title.x = element_blank(),
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
...
```{r}
plot_grid(plot1, plot2, labels = c('Temperature', 'Precipitation'), ncol = 1, rel_heights = c(2,1), align = "v", label_size = 10)
```
